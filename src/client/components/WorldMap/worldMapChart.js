...
world.create = function (el, props) {
  ...
  g.append("use").attr("xlink:href", "#land");

  const defs = g.append("defs");

  g.append("path").attr("id", "land").attr("class", "land");

  const overlayGroup = g
    .append("g")
    .attr("class", "overlay")
    .attr("clip-path", "url(#clip)");

  const graticuleGroup = g.append("g").attr("class", "graticules");
  graticuleGroup.append("path").attr("class", "graticule");
  graticuleGroup.append("path").attr("class", "graticule-outline");
...
world.update = function (el, properties) {
  ...
  svg.call(zoom).on("click", onClick);
  ...
  landPath.style("fill", "white").style("stroke", "#666");
  ...
  function drawRawFeatures() {
    ...
    svgFeature.attr("d", geoPath).style("fill", "none").style("stroke", "red");
  }
  ...
  if (props.bins.length > 0) {
    ...
    const strokeColor = props.selectedCell
      ? selectedStrokeColor
      : ordinaryStrokeColor;

    const strokeOpacity = (d) =>
      props.selectedCell && props.selectedCell === d ? 1.0 : 0.5;

    const strokeWidth = (d) => {
      const unitFactor = props.binning.unit === Binning.MINUTE ? 0.02 : 0.1;
      const highlightFactor =
        props.selectedCell && props.selectedCell === d ? 2 : 1;
    ...
    const getClusterColor = (d) => {
      ...
      if (props.selectedClusterId >= 0) {
        return d.clusterId === props.selectedClusterId
          ? clusterColor.css()
          : chroma(clusterColor.hex()).alpha(0.2).css();
    ...
    //Update
    binPaths
      .attr("d", props.binning.renderer(props.projection))
      .style("fill", color)
    ...
    let binPaths = g.select(".overlay").selectAll(".bin").data(props.bins);
