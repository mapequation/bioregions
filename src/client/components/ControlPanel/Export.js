...
class Export extends Component {
  ...
  render() {
    if (this.props.bins.length === 0) return <span></span>;

    return (
      <Div paddingTop="10px">
        <button className="ui button" onClick={this.showExport}>
          Export...
        </button>
...
var DownloadButton = ({ onClick, label, type }) => (
  <button className="ui very basic compact button" onClick={onClick}>
    {label}
...
class ExportWindow extends Component {
  ...
  getSvg(elementId) {
    return new Promise((resolve) => {
      ...
      if ($svg.length === 0) return resolve(null);
  ...
  getClustersCSV() {
    return new Promise((resolve) => {
      ...
      clusters.forEach((cluster) => {
        ...
        R.zip(topCommonSpecies, topIndicatorSpecies).forEach(
          ([common, indicator], i) => {
            rows.push({
  ...
  getNetwork() {
    return new Promise((resolve) => {
      ...
      // resolve(getBipartiteNetwork({ species, bins, speciesToBins }));
      // if (!phyloTree) {
      //   resolve(getPajekNetwork(species, speciesToBins, bins));
      // } else {
      ...
      >
        <div className="ui container" style={{ background: "white" }}>
          ...
          <div className="ui two column grid">
            <div className="column">{FileGroups}</div>
