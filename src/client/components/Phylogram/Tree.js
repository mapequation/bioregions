...
class Tree extends Component {
  ...
  updateTree(newPropsOrState) {
    ...
    treeChart.update(this.svg, state);
    treeChart.update(
      this.minimap,
      Object.assign({}, state, {
        minimap: true,
  ...
  getFilteredTree(state) {
    if (!state.phyloTree) return null;
    const comparator = {
      ...
      [BY_BIOREGIONS]: (a, b) => {
        ...
        if (aclu.length === 0 || bclu.length === 0) return 0;
  ...
  renderControlPanel() {
    ...
    const sortOptions = [
      BY_ORIGINAL_ORDER,
      BY_MAX_BRANCH_LENGTH_DESC,
      BY_MAX_BRANCH_LENGTH_ASC,
      BY_BRANCH_SIZE_DESC,
      BY_BRANCH_SIZE_ASC,
    ];
    ...
    console.log(
      `[Tree]: haveGeoSpecies: ${haveGeoSpecies}, haveClusters: ${haveClusters}`,
    );
  ...
  render() {
    ...
    return (
      <Div
        ...
        display={phyloTree ? 'block' : 'none'}
      >
        <div className="four wide column">
          <div className="ui segment">
            <h4 className="ui header">Tree</h4>
            <svg id="treeMinimap" ref={(el) => (this.minimap = el)}></svg>
            {this.renderControlPanel()}
          </div>
        </div>
        <div className="twelve wide column">
          <div className="ui segment main-tree-container">
            <svg id="phylogram" ref={(el) => (this.svg = el)}></svg>
          </div>
        </div>
      </Div>
    );
